<template>
	<div class="yinhuan">
		<van-nav-bar title="隐患整改通知" left-text="返回" right-text="操作" left-arrow @click-left="pageBack" @click-right="openAction"/>
		<div class="cell_group">
			...
			<!-- 整改负责人 -->
			<cell-select-users title="整改负责人" required :storeModule="storeModule" storeKey="rectificationCharge" v-model="rectificationCharge"></cell-select-users>
			<!-- 地点 -->
			...
			<!-- 隐患来源 -->
			<cell-picker title="隐患来源" required v-model="dangerSource" :columns="dangerSourceColumns" keyType="text"></cell-picker>
			<!-- 是否处罚 -->
			...
			<!-- 验收负责人 -->
			<cell-select-users title="验收负责人" required :storeModule="storeModule" storeKey="acceptanceCharge" v-model="acceptanceCharge"></cell-select-users>
		</div>
			...
			getDataInfo() {
				...
				this.$api.page_3
					...
					.then(res => {
						...
						this.isPunish = parseInt(info.isPunish || 0) + 1;
						this.dangerLevel = parseInt(info.dangerLevel || 0) + 1;
						this.dangerType = parseInt(info.dangerType || 0) + 1;
						this.dangerSource = parseInt(info.dangerSource || 0) + 1;
						this.punishMoney = info.punishMoney || "";
