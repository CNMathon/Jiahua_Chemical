<template>
  <div class="home">
    <van-nav-bar title="电气第二种工作票" left-text="返回" left-arrow @click-left="pageBack" fixed />
    <j-filter-bar
      v-model="searchValue"
      placeholder="请输入作业名称"
      ...
      <div class="app">
        <van-skeleton title :row="5" :loading="isFirstLoading" class="skeleton">
          <van-pull-refresh v-model="isRefreshLoading" @refresh="getPageData('refresh')">
            ...
            >-->
            <dianqi_2ListCase></dianqi_2ListCase>
          </van-pull-refresh>
        </van-skeleton>
      </div>
    </div>
...
import dianqi_2ListCase from "./components/List.vue";
export default {
  name: 'dianqi_2_list',
  mixins: [mixin],
  components: {dianqi_2ListCase},
  data() {
    return {
      searchValue: "",
      showFilter: false,
