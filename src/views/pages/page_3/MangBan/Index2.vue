<template>
	<div class="mangban">
		<van-nav-bar title="盲板抽堵安全" left-text="返回" right-text="操作" left-arrow @click-left="pageBack" @click-right="openAction" />
		...
		<div class="cell_group" v-for="(item, index) in sendData.pipe" :key="index">
			...
			<!-- 材质 -->
			<cell-picker title="材质" v-model="sendData.pipe[index].pipeMaterial" :columns="materialColumns" />
			<!-- 规格 -->
			<div class="cell">
				<div class="cell_title">
					<span>规格</span>
			...
			<!-- 堵作业人 -->
			<cell-select-user title="堵作业人" :storeModule="storeModule" :storeKey="'pipeBlockOperator'+ index" v-model="sendData.pipe[index].pipeBlockOperator" />
			<!-- 抽作业人 -->
			<cell-select-user title="抽作业人" :storeModule="storeModule" :storeKey="'pipePullOperator'+ index" v-model="sendData.pipe[index].pipePullOperator" />
			<!-- 堵监护人 -->
			<cell-select-user title="堵监护人" :storeModule="storeModule" :storeKey="'pipeBlockGuardian'+ index" v-model="sendData.pipe[index].pipeBlockGuardian" />
			<!-- 抽监护人 -->
			<cell-select-user title="抽监护人" :storeModule="storeModule" :storeKey="'pipePullGuardian'+ index" v-model="sendData.pipe[index].pipePullGuardian" />
		...
		<div class="cell_group">
			<!-- 生产部门作业负责人 -->
			<cell-select-user title="生产部门作业负责人" required :storeModule="storeModule" storeKey="scMan" v-model="sendData.scMan"></cell-select-user>
			<!-- 作业部门负责人 -->
			<cell-select-user title="作业部门负责人" required :storeModule="storeModule" storeKey="zyMan" v-model="sendData.zyMan"></cell-select-user>
			<!-- 涉及其他特殊作业 -->
			<cell-select-tag required title="涉及其他特殊作业" storeKey="otherSpecial" :tagList="otherSpecial" :showList="list_1"
			 :storeModule="storeModule"></cell-select-tag>
		</div>
		<!-- 时间选择 -->
		<van-popup v-model="timeShow" position="bottom">
			<van-datetime-picker v-model="currentDate" type="datetime" :min-date="new Date()" @cancel="onTimeCancel" @confirm="onTimeConfirm" />
		</van-popup>
		...
		methods: {
			...
			postData() {
				...
				for (let i = 0; i <= 3; i++) {
					...
					dataStrArr[i] =
						`${pipe[i].pipeName}|${pipe[i].pipeMedium}|${
          pipe[i].pipePressure
