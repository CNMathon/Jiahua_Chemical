<template>
  <div class="mangban">
    ...
    <div class="cell_group" v-for="(item, index) in sendData.pipe" :key="index">
      ...
      <!-- 设备管道名称 -->
      <cell-input title="设备管道名称" v-model="item.pipeName" placeholder="手工录入"></cell-input>
      <!-- 介质 -->
      <cell-input title="介质" v-model="item.pipeMedium" placeholder="手工录入"></cell-input>
      <!-- 温度 -->
      <cell-input title="温度" v-model="item.pipeTemp" placeholder="手工录入"></cell-input>
      <!-- 压力 -->
      <cell-input title="压力" v-model="item.pipePressure" placeholder="手工录入"></cell-input>
      <!-- 材质 -->
      <cell-picker
        title="材质"
      ...
      />
      <!-- 规格 -->
      <cell-input title="规格" v-model="item.pipeSpec" placeholder="手工录入"></cell-input>
      <!-- 编号 -->
      <cell-input title="编号" v-model="item.pipeNumber" placeholder="手工录入"></cell-input>
      <!-- 堵时间 -->
    ...
    <div class="confirm">
      ...
      <div class="confirm_list">
        <div v-for="(item,index) in checked" :key="index">
          <Signature
...
export default {
  ...
  methods: {
        ...
        .then(res => {
          ...
          for (const key in info) {
            if (key === "scMan") {
              ...
              let arr = info[key].split("|");
              this.filterPipeUser(this.sendData.pipe[0], arr);
            } else if (key === "twoPipe") {
              if (info[key].length > 14) {
                let arr = info[key].split("|");
                this.filterPipeUser(this.sendData.pipe[1], arr);
              }
            } else if (key === "threePipe") {
              if (info[key].length > 14) {
                let arr = info[key].split("|");
                this.filterPipeUser(this.sendData.pipe[2], arr);
              }
            } else if (key === "fourPipe") {
              if (info[key].length > 14) {
                let arr = info[key].split("|");
                this.filterPipeUser(this.sendData.pipe[3], arr);
              }
            }
          }
    ...
    filterPipeUser(info, arr) {
      ...
      info.pipePullOperator = this.reductionSelectUser(arr[9]);
      info.pipeBlockGuardian = this.reductionSelectUser(arr[10]);
      info.pipePullGuardian = this.reductionSelectUser(arr[11]);
    },
    // 保存签名
    saveCanvas(e) {
