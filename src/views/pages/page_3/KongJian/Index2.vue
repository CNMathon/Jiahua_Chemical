  ...
  <div class="donghuo">
    ...
    <div class="cell_group">
      ...
      <cell-value title="作业票编号"
                  ...
                  :value="$userInfo.dhzypCode"></cell-value>
      <!-- 作业票状态 -->
      ...
      <cell-textarea title="作业内容"
                     ...
                     placeholder="请输入作业内容"
                     disabled
                     class="disabled"></cell-textarea>
      <!-- 设备名称 -->
      <cell-value title="设备名称"
                  required
                  v-model="sendData.devicename"
                  placeholder="请输入设备名称"
                  disable></cell-value>
      <!-- 受限空间原有界质 -->
      <cell-value title="受限空间原有界质"
                  required
                  v-model="sendData.sxkjNeurogen"
                  placeholder="请输入界质名称"
                  disable></cell-value>
      <!-- 涉及其他特殊作业 -->
      <cell-select-tag disable
                       title="涉及其他特殊作业"
      ...
      <div class="confirm">
        ...
        <div class="confirm_list">
          ...
          <Signature :checked="checked[10] ? checked[10].checked : false"
      ...
      <div class="fenxi"
        ...
        <div class="kongjianFenxi">
          ...
          <div v-for="(fxData, fxindex) in fx.fenxidata"
               :key="fxindex + 'fxData'"
               class="kongjianFenxi-box">
            <div class="kongjianFenxi-title">{{fxData.title}}</div>
            <div class="kongjianFenxi-standard">
              <div v-for="(fxStandard, standardindex) in fxData.standard"
            ...
            <div class="kongjianFenxi-data">
              <div v-for="(fxDataData, datadataindex) in fxData.data"
                   :key="datadataindex + 'fxDataData'">
                <van-field :value="fxDataData"
        ...
        <!-- 分析时间 -->
        <cell-time v-model="fx.fenxiTime"
                   title="动火分析时间"
                   disable></cell-time>
        <!-- 分析点名称 -->
        <cell-input v-model="fx.fenxiParts"
                    title="分析部位"
                    required
                    placeholder="手工录入"></cell-input>
        <!-- 分析人 -->
        <div class="kongjianFenxiRen"
             @click="showFenxiRensignature(index)">
          <div>分析人<span class="redColor">*</span></div>
          <div><img v-if="fx.fenxiren"
                 :src="fx.fenxiren"
                 class="kongjianFenxiRen-img"></div>
        </div>
      </div>
    ...
    <!--  分析选择 -->
    <van-popup v-model="showFenxiSelect"
               position="bottom">
    ...
    methods: {
      Next () {
        ...
        if (this.actRuTask === '') {
          ...
          this.$api.page_3.start('sxkjzyp/htHseSxkjzyp', data).then((res) => {
            if(res.result==='true'){
              console.log(res)
              if (res.groups) {
                this.$router.push({
