<template>
  <div class="donghuo">
    <van-sticky>
      ...
      />
      <van-nav-bar v-else title="受限空间安全" left-text="返回" left-arrow @click-left="pageBack" />
    </van-sticky>
    <div class="cell_group">
      ...
      <!-- 作业票编号 -->
      <cell-value title="作业票编号" :value="oldInfo.sxkjCode" disable></cell-value>
      <!-- 作业票编号 -->
      <cell-value title="作业票编号" required v-if="initData.dhzypCode" :value="$userInfo.dhzypCode"></cell-value>
      <!-- 作业票状态 -->
      <cell-value title="作业票状态" required :value="zypztList[oldInfo.htStatus].name" disable></cell-value>
      <!-- 受限空间所属空间 -->
      ...
      <div class="fenxi" v-for="(fx, index) in fenxi" :key="index">
        ...
        <!-- 分析点名称 -->
        <cell-input v-model="fx.fenxiParts" title="分析部位" required disable="true" placeholder="手工录入"></cell-input>
        <!-- 分析人 -->
