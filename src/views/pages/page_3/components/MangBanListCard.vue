<template>
	<div class="app">
		<van-skeleton title :row="3" :loading="isLoading" class="skeleton" v-for="(item, index) in 3" :key="index">
		</van-skeleton>
		<label v-for="(item, index) in listData" :key="index">
			...
			<div class="donghuo-list-card donghuo-list-card-nolast">
				...
				<div class="right" @click.stop="()=>{$router.push({path:'/page_3/mangban/index',query:{id:item.id}})}" v-if="item.htStatus == 1">编辑</div>
				<div class="right" v-if="item.htStatus == 2">提交资料</div>
				<div class="right" @click.stop="()=>{$router.push({path:'/page_3/mangban/index2',query:{id:item.id}})}" v-if="item.htStatus == 3">初审</div>
