<template>
	<div class="mangban">
		<van-sticky>
			<van-nav-bar title="高处安全" v-if="htStatus!=='5'" left-text="返回" right-text="操作" left-arrow @click-left="pageBack" @click-right="showPicker = true" />
      <van-nav-bar title="高处安全" v-else left-text="返回" left-arrow @click-left="pageBack"/>
		</van-sticky>
		<div class="cell_group">
			<!-- 申请部门 -->
			<cell-value title="申请部门" disable :value="$userInfo.officeName" disable></cell-value>
			<!-- 申请人 -->
			<cell-value title="申请人" disable :value="$userInfo.userName" disable></cell-value>
			<!-- 作业票编号 -->
			<cell-value title="作业票编号" disable :value="sendData.id" disable></cell-value>
			<!-- 作业票状态 -->
			<cell-value title="作业票状态" disable value="编辑" disable></cell-value>
			<!-- 作业内容 -->
			<cell-textarea title="作业内容" disable required v-model="sendData.workContent" placeholder="请输入工作内容"></cell-textarea>
			<!-- 作业地点 -->
			<cell-input title="作业地点" disable required v-model="sendData.workAddress" placeholder="手工录入"></cell-input>
			<!-- 作业高度 -->
			<cell-picker title="作业高度" disable required v-model="sendData.workHeight" :columns="workHeightColumns"></cell-picker>
			<!-- 登高类别 -->
			<cell-picker title="登高类别" disable required v-model="sendData.heightType" :columns="heightTypeColumns"></cell-picker>
			<!-- 涉及其他特殊作业 -->
			<cell-select-tag disable required title="涉及其他特殊作业" storeKey="specialWork" :tagList="sendData.specialWork" :showList="list_1"
			 :storeModule="storeModule"></cell-select-tag>
			<!-- 危害辨识 -->
			<cell-select-tag disable required title="危害辨识" storeKey="harmAnalise" :tagList="sendData.harmAnalise" :showList="list_2"
			 :storeModule="storeModule"></cell-select-tag>
			<!-- 作业开始时间 -->
			...
			<!-- 作业部门负责人 -->
			<cell-select-user disable title="作业部门负责人" required :storeModule="storeModule" :value="sendData.workDeptLeader" storeKey="workDeptLeader"
			 v-model="sendData.workDeptLeader"></cell-select-user>
			<!-- 监护人 -->
			<cell-select-user disable title="监护人" required :storeModule="storeModule" storeKey="guarder" v-model="sendData.guarder"></cell-select-user>
			<!-- 作业人 -->
			<cell-select-user disable title="作业人" required :storeModule="storeModule" storeKey="worker" v-model="sendData.worker"></cell-select-user>
		</div>
		...
		<div class="confirm">
			...
			<div class="confirm_list">
				...
				<Signature disable :checked="checked[6] ? checked[6].checked : false" :img="checked[6] ? checked[5].img : ''" @checked="showSignature(6)"
	...
	export default {
      ...
      cancelCanvas2() {
        ...
        if (this.actRuTask === '') {
          ...
          this.$api.page_3.start('heightworkticket/htHseUpworkticket', data).then((res) => {
            if(res.result==='true'){
              console.log(res)
              if (res.groups) {
                this.$router.push({
