  ...
  <div class="donghuo">
    ...
    <div class="cell_group fixed-first">
      ...
      <cell-value title="作业票状态"
                  ...
                  disable></cell-value>
      <!-- 动火地点及内容 -->
      <cell-textarea title="动火地点及内容"
                     required
                     v-model="sendData.siteContent"
                     placeholder="请输入作业内容"
                     disable></cell-textarea>
      <!-- 动火级别 -->
      <cell-value title="动火级别"
                  :value="dhLevelColumns[sendData.dhLevel - 1]"
                  required
                  disable
                  arrow></cell-value>
      <!-- 动火方式 -->
      ...
      <cell-select-tag required
                       ...
                       disable></cell-select-tag>
      <!-- 动火开始时间 -->
      <cell-time v-model="sendData.dhStarttime"
                 title="动火开始时间"
                 required
                 disable></cell-time>
      <!-- 动火结束时间 -->
      <cell-time v-model="sendData.dhEndtime"
                 title="动火结束时间"
                 required
                 disable></cell-time>
      <!-- 动火作业负责人 -->
      <cell-select-user title="动火作业负责人"
                        required
                        :storeModule="storeModule"
                        storeKey="dhzyPrincipal"
                        v-model="sendData.dhzyPrincipal"
                        disable></cell-select-user>
      <!-- 动火人 -->
      <cell-select-user title="动火人"
                        required
                        :storeModule="storeModule"
                        storeKey="dhzyRen"
      ...
      <div class="fenxi">
        ...
        <!-- 动火分析时间 -->
        <cell-time v-model="sendData.dhfxTimeA"
                   title="动火分析时间"
                   required></cell-time>
        <!-- 分析点名称 -->
        <cell-input v-model="sendData.fxdNameA"
                    title="分析点名称"
                    required
                    placeholder="手工录入"></cell-input>
        <!-- 可燃气体爆炸极限 -->
        <cell-picker title="可燃气体爆炸极限"
                     required
                     v-model="sendData.krqbzLimitationA"
                     :columns="krqbzLimitationAColumns"></cell-picker>
        <!-- 分析数据 -->
        <cell-input v-model="sendData.fxDataA"
                    title="分析数据"
                    required
                    placeholder="手工录入"></cell-input>
        <!-- 分析人 -->
        <cell-select-user title="分析人"
                          required
                          :storeModule="storeModule"
                          radio
                          storeKey="fxRenA"
                          v-model="sendData.fxRenA"></cell-select-user>
      </div>
      <div class="fenxi"
           v-if="sendData.dhfxTimeB !== undefined">
        <div class="fenxi__title">动火分析2</div>
        <!-- 动火分析时间 -->
        <cell-time v-model="sendData.dhfxTimeB"
                   title="动火分析时间"
                   required></cell-time>
        <!-- 分析点名称 -->
        <cell-input v-model="sendData.fxdNameB"
                    title="分析点名称"
                    required
                    placeholder="手工录入"></cell-input>
        <!-- 可燃气体爆炸极限 -->
        <cell-picker title="可燃气体爆炸极限"
                     ...
                     :columns="krqbzLimitationAColumns"></cell-picker>
        <!-- 分析数据 -->
        <cell-input v-model="sendData.fxDataB"
                    title="分析数据"
                    required
                    placeholder="手工录入"></cell-input>
        <!-- 分析人 -->
        <cell-select-user title="分析人"
                          ...
                          radio
                          storeKey="fxRenB"
                          v-model="sendData.fxRenB"></cell-select-user>
      </div>
      <div class="fenxi"
           v-if="sendData.dhfxTimeC !== undefined">
        <div class="fenxi__title">动火分析3</div>
        <!-- 动火分析时间 -->
        <cell-time v-model="sendData.dhfxTimeC"
                   title="动火分析时间"
                   required></cell-time>
        <!-- 分析点名称 -->
        <cell-input v-model="sendData.fxdNameC"
                    title="分析点名称"
                    required
                    placeholder="手工录入"></cell-input>
        <!-- 可燃气体爆炸极限 -->
        <cell-picker title="可燃气体爆炸极限"
                     ...
                     :columns="krqbzLimitationAColumns"></cell-picker>
        <!-- 分析数据 -->
        <cell-input v-model="sendData.fxDataC"
                    title="分析数据"
                    required
                    placeholder="手工录入"></cell-input>
        <!-- 分析人 -->
        <cell-select-user title="分析人"
                          ...
                          radio
                          storeKey="fxRenC"
                          v-model="sendData.fxRenC"></cell-select-user>
      </div>

      <div class="adddonghuofenxi"
           @click="addFenxi">
        <van-icon name="plus" />
      </div>
      <!-- 其他安全措施 -->
      <cell-textarea title="其他安全措施"
                     required
                     placeholder="请输入其他安全措施"
                     v-model="sendData.otherSafety"></cell-textarea>
      <div class="signature"
           @click="signatureShow = true">
        <div class="signature__head">
          ...
          <div class="signature__icon"
               v-if="signatureA">{{signatureATime}}</div>
          <van-icon class="signature__icon"
                    v-else
                    name="edit" />
        </div>
      </div>
    ...
    methods: {
          ...
          .then(res => {
            ...
            for (const key in this.sendData) {
              if (key === "dhzyPrincipal" && info.dhfzr) {
                ...
                console.log('dhzyRen=========', key && info.dhr);
      ...
      Next () {
        if (this.actRuTask === '') {
          ...
          this.$api.page_3.start('dhzyp/htHseDhzyp', data).then((res) => {
            console.log(res)
            if (res.groups) {
              this.$router.push({
