...
const article = {
  ...
  htDeviceDefectForm(params) {
    return axios.get(`${base.sq}device/defect/htDeviceDefect/form.json`, {
      params: params
    });
  },
  // 设备缺陷保存
  ...
  // 消息通知
  actCoreMobileMsgListData(params) {
    return axios.get(`${base.sq}act/core/mobile/msgListData?${qs.stringify(params)}`);
  },
  // 已推送消息成功数
  msgUnreadMsg(params) {
    return axios.post(
      `${base.sq}msg/unreadMsg`,
      qs.stringify(params)
    );
  },
  // 已推送消息成功数
